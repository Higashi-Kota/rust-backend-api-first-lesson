...
use task_backend::{
    api::{
        ...
        handlers::task_handler::task_router_with_schema,
    },
...
// 統合テスト用のヘルパー関数: テスト専用のスキーマとルーターを作成
async fn setup_test_app() -> (Router, String, common::db::TestDatabase) {
    // 新しいテストデータベースを作成（一意のスキーマで）
    let db = common::db::TestDatabase::new().await;
    let schema_name = db.schema_name.clone();

    // スキーマを指定してTaskServiceを作成
    let service = std::sync::Arc::new(TaskService::with_schema(
        db.connection.clone(),
        schema_name.clone(),
    ));

    // スキーマを指定してルーターを作成
    let app = task_router_with_schema(service);

    (app, schema_name, db)
}

// テスト用のタスクを作成するヘルパー関数
async fn create_test_task(app: &Router) -> TaskDto {
...
async fn test_get_task_endpoint() {
    ...
    // まずタスクを作成
    let created_task = create_test_task(&app).await;
...
async fn test_list_tasks_endpoint() {
    ...
    assert_eq!(tasks.len(), 3); // 正確に3つのタスクが取得されるはず
}
...
async fn test_filter_endpoint() {
    ...
    assert_eq!(filtered_tasks.len(), 2); // todoタスクは2つあるはず
    ...
    let filtered_tasks = result["tasks"].as_array().unwrap();
    assert_eq!(filtered_tasks.len(), 2); // "Important"を含むタスクは2つあるはず
    ...
    let filtered_tasks = result["tasks"].as_array().unwrap();
    assert_eq!(filtered_tasks.len(), 2); // todoかつ"Important"を含むタスクは2つあるはず
    ...
    let filtered_tasks = result["tasks"].as_array().unwrap();
    assert!(filtered_tasks.is_empty()); // 該当するタスクはないはず
}
