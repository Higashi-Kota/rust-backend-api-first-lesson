...
/// API レート制限テスト
#[tokio::test]
#[ignore = "Rate limiting requires stateful middleware architecture"]
async fn test_api_rate_limit_by_tier() {
    // Arrange
    let (app, _schema_name, _db) = app_helper::setup_full_app().await;
    let free_user = auth_helper::setup_authenticated_user(&app).await.unwrap();

    // Act - Make rapid API calls
    ...
    // Assert - Free tier should hit rate limit
    assert!(rate_limited || success_count < 100);
}

/// 動的権限によるレスポンスフィルタリングテスト
#[tokio::test]
#[ignore = "Response filtering not implemented yet"]
async fn test_dynamic_response_filtering() {
    // Arrange
    let (app, _schema_name, _db) = app_helper::setup_full_app().await;
    let user = auth_helper::setup_authenticated_user(&app).await.unwrap();

    // Create tasks with various priorities
    let high_priority_task = json!({
    ...
    let create_req = auth_helper::create_authenticated_request(
        ...
        "/tasks",
        &user.access_token,
        Some(serde_json::to_string(&high_priority_task).unwrap()),
    );

    app.clone().oneshot(create_req).await.unwrap();

    // Act - Get tasks (free tier might not see priority)
    let list_req =
    ...
    // Assert - Check if priority field is filtered for free tier
    if let Some(tasks) = tasks_response["data"].as_array() {
        for task in tasks {
            // Free tier might not have access to priority field
            // Check if priority field exists (implementation-dependent for free tier)
            let _has_priority = task.get("priority").is_some();
            // Free tier might have priority hidden, so we don't assert on it
        }
    }
}
