...
impl BulkOperationHistoryRepository {
    ...
    pub async fn create(
        ...
        performed_by: Uuid,
        affected_count: i32,
    ) -> AppResult<crate::features::admin::models::bulk_operation_history::Model> {
        let model = crate::features::admin::models::bulk_operation_history::Model::new(
            operation_type,
            performed_by,
            affected_count,
        );

        let active_model = crate::features::admin::models::bulk_operation_history::ActiveModel {
            id: Set(model.id),
    ...
    pub async fn get_by_id(
        &self,
        id: Uuid,
    ) -> AppResult<Option<crate::features::admin::models::bulk_operation_history::Model>> {
    ...
    pub async fn get_by_user(
        ...
        let operations = BulkOperationHistory::find()
            .filter(
                crate::features::admin::models::bulk_operation_history::Column::PerformedBy
                    .eq(user_id),
            )
            .order_by_desc(
                crate::features::admin::models::bulk_operation_history::Column::CreatedAt,
            )
            .limit(limit)
            .all(&self.db)
            .await?;
