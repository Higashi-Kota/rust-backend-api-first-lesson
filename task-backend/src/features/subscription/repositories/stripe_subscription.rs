// src/features/subscription/repositories/stripe_subscription.rs
...
use super::super::models::stripe_subscription::{
...
impl StripeSubscriptionRepository {
    ...
    ) -> Result<Option<stripe_subscription::Model>, DbErr> {
        self.prepare_connection().await?;
        SubscriptionEntity::find()
            .filter(stripe_subscription::Column::StripeSubscriptionId.eq(stripe_subscription_id))
            .one(&self.db)
            .await
    }
    ...
    ) -> Result<stripe_subscription::Model, DbErr> {
        self.prepare_connection().await?;
