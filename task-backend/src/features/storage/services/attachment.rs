// task-backend/src/features/storage/attachment/service.rs

use crate::core::subscription_tier::SubscriptionTier;
use crate::db::DbPool;
use crate::error::{AppError, AppResult};
use crate::features::storage::repositories::attachment_repository::{
    AttachmentRepository, CreateAttachmentDto,
};
use crate::features::storage::repositories::attachment_share_link_repository::{
    AttachmentShareLinkRepository, CreateShareLinkDto,
};
use crate::features::task::models::task_attachment_model::{
    get_all_allowed_mime_types, is_allowed_mime_type, MAX_FILE_SIZE_ENTERPRISE, MAX_FILE_SIZE_FREE,
    MAX_FILE_SIZE_PRO,
};
...
use crate::features::storage::services::{sanitize_filename, StorageService};
...
impl AttachmentService {
    ...
    pub async fn upload_file(
                ...
                match optimize_image(
                    ...
                    Ok(result) => {
                        ...
                        let original_key = if optimization_config.keep_original {
                            let original_storage_key =
                                format!("attachments/{}/{}-original", task_id, Uuid::new_v4());
                            let key = self
                                .storage
                                .store(&original_storage_key, &file_data, &mime_type)
                                .await
                                .map_err(|e| {
        ...
        // ストレージにアップロード
        let storage_key = format!("attachments/{}/{}", task_id, Uuid::new_v4());
        self.storage
            .store(&storage_key, &final_data, &final_mime_type)
            .await
        ...
        let attachment = self
            ...
            .map_err(|e| {
                ...
                let original_key = original_storage_key;
    ...
    pub async fn download_attachment(
        ...
        let file_data = self
            .storage
            .retrieve(&attachment.storage_key)
            .await
            .map_err(|e| {
