// task-backend/src/features/auth/handler.rs
use crate::api::{AppState, CookieConfig, HasJwtManager, SecurityHeaders};
use crate::error::{AppError, AppResult};
use crate::features::auth::dto::*;
use crate::features::auth::handlers::middleware::AuthenticatedUser;
use axum::{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let access_claims = state
            ...
            .map_err(|e| {
                ...
                match e {
                    crate::infrastructure::jwt::JwtError::TokenExpired => {
                        AppError::Unauthorized("Access token has expired".to_string())
                    }
                    crate::infrastructure::jwt::JwtError::InvalidToken => {
...
pub async fn change_password_handler(
    ...
    payload.validate_password_change().map_err(|e| {
        ...
        AppError::ValidationErrors(vec![e.to_string()])
    })?;

    info!(user_id = %user.claims.user_id, "Password change attempt");

    // パスワード変更用の構造体に変換
    let change_input = crate::infrastructure::password::PasswordChangeInput {
...
fn create_auth_cookies(
    tokens: &crate::infrastructure::jwt::TokenPair,
    config: &CookieConfig,
) -> CookieJar {
...
pub fn auth_router(app_state: AppState) -> Router {
    use crate::features::auth::middleware::rate_limit_middleware;
